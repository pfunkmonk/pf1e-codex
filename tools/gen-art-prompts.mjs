...
/* Emit data/artplan.js — every key that should EVENTUALLY exist. The art gallery used to iterate a
 * hand-maintained ART_PLANNED literal inside app.js; the moment the theme system began generating
 * keys that list went stale, and the gallery would have shown 1,652 of 3,758 while reporting the
 * other 2,106 as not even planned. Deriving it here keeps one source of truth: whatever is on disk,
 * plus everything the declarations imply, plus everything the packs commission. */
{
  const plan = new Set(globalThis.window.PF_ART || []);
  for (const [b, t] of Object.entries(THEMES)) for (const r of t)
    for (let v = 1; v <= (r[3] || 1); v++) plan.add(`theme-${b}-${r[0]}-${v}`);
  for (const [b, t] of Object.entries(globalThis.window.PF_BODY_THEMES || {})) for (const r of t) {
    if (r[3]) continue;                                   // reuse row: owns no key of its own
    for (let v = 1; v <= (r[2] || 1); v++) plan.add(`theme-${b}-${r[0]}-${v}`);
  }
  for (const [n, c] of Object.entries(VARIETY)) for (let v = 1; v <= c; v++) plan.add(`${n}-${v}`);
  for (const n of Object.keys(BATCHES)) for (const it of buildBatch(Number(n))) plan.add(it.key);
  const sorted = [...plan].sort();
  const header = [
    "/* GENERATED by tools/gen-art-prompts.mjs - do not edit by hand.",
    " * Every art key that should eventually exist: on disk, declared, or commissioned in a pack.",
    " * Loaded lazily by the art gallery only; it is not on the cold-start path. */",
    "window.PF_ART_PLAN=" + JSON.stringify(sorted) + ";",
    ""
  ].join(String.fromCharCode(10));
  fs.writeFileSync(path.join(ROOT, "data/artplan.js"), header);
  console.log(`
data/artplan.js: ${sorted.length} planned keys`);
}
